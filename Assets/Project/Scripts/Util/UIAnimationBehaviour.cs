    ...
    public class UIAnimation
    {
        ...
        public IEnumerator AnimateUIElement(UIAnimation p_uiAnimation, UIAnimationType p_uiAnimationType)
        {
            ...
            bool l_isFadeComplete = !p_uiAnimation.shouldFade,
                l_isMoveComplete = !p_uiAnimation.shouldMove,
                l_isScaleComplete = !p_uiAnimation.shouldScale;
            ...
            while (!l_isFadeComplete || !l_isMoveComplete || !l_isScaleComplete)
            {
                if (!l_isFadeComplete)
                {
                    if (l_timeCounter >= p_uiAnimation.fadeDelayTime)
                    {
                        ...
                        if (l_usesCurve)
                        {
                            l_canvasGroup.alpha = p_uiAnimation.fadeAnimationCurve.Evaluate(
                                Mathf.LerpUnclamped(p_uiAnimation.startAlpha, l_isFadeIn ? 1 : 0,
                                    l_fadeTimeCounter / p_uiAnimation.fadeAnimationTime));
                        }
                        else
                        {
                            l_canvasGroup.alpha = Mathf.LerpUnclamped(p_uiAnimation.startAlpha, l_isFadeIn ? 1 : 0,
                                l_fadeTimeCounter / p_uiAnimation.fadeAnimationTime);
                        }
                    ...
                    if (l_timeCounter >= p_uiAnimation.moveDelayTime)
                    {
                        ...
                        if (l_usesCurve)
                        {
                            if (p_uiAnimation.useLocalPosition)
                                p_uiAnimation.uiElement.anchoredPosition = Vector3.LerpUnclamped(l_initialPosition,
                                    p_uiAnimation.finalLocalPosition,
                                    p_uiAnimation.moveAnimationCurve.Evaluate(l_moveTimeCounter /
                                                                              p_uiAnimation.moveAnimationTime));
                            else
                                p_uiAnimation.uiElement.transform.position = Camera.main.ViewportToScreenPoint(
                                    Vector3.LerpUnclamped(l_initialPosition, p_uiAnimation.finalPosition,
                                        p_uiAnimation.moveAnimationCurve.Evaluate(l_moveTimeCounter /
                                            p_uiAnimation.moveAnimationTime)));
                        }
                        else
                        {
                            if (p_uiAnimation.useLocalPosition)
                                p_uiAnimation.uiElement.anchoredPosition = Vector3.LerpUnclamped(l_initialPosition,
                                    p_uiAnimation.finalLocalPosition,
                                    l_moveTimeCounter / p_uiAnimation.moveAnimationTime);
                            else
                                p_uiAnimation.uiElement.transform.position = Camera.main.ViewportToScreenPoint(
                                    Vector3.LerpUnclamped(l_initialPosition, p_uiAnimation.finalPosition,
                                        l_moveTimeCounter / p_uiAnimation.moveAnimationTime));
                ...
                if (!l_isScaleComplete)
                {
                    if (l_timeCounter >= p_uiAnimation.scaleDelayTime)
                    {
                        ...
                        if (l_usesCurve)
                        {
                            p_uiAnimation.uiElement.localScale = Vector3.LerpUnclamped(p_uiAnimation.startScale,
                                p_uiAnimation.finalScale,
                                p_uiAnimation.scaleAnimationCurve.Evaluate(l_scaleTimeCounter /
                                                                           p_uiAnimation.scaleAnimationTime));
                        }
                        else
                        {
                            p_uiAnimation.uiElement.localScale = Vector3.LerpUnclamped(p_uiAnimation.startScale,
                                p_uiAnimation.finalScale, l_scaleTimeCounter / p_uiAnimation.scaleAnimationTime);
                        }
            ...
            if (p_uiAnimationType.Equals(UIAnimationType.ENTRY))
            {
                ...
                if (gameObject.activeInHierarchy)
                {
                    for (int i = 0; i < entryUiAnimationsList.Count; i++)
                    {
                        ...
                        entryUiAnimationsList[i].animCoroutine = StartCoroutine(entryUiAnimationsList[i]
                            .AnimateUIElement(entryUiAnimationsList[i], UIAnimationType.ENTRY));
                    }
                }
            }
            else
            {
                ...
                if (gameObject.activeInHierarchy)
                {
                    for (int i = 0; i < leaveUiAnimationsList.Count; i++)
                    {
                        ...
                        leaveUiAnimationsList[i].animCoroutine = StartCoroutine(leaveUiAnimationsList[i]
                            .AnimateUIElement(leaveUiAnimationsList[i], UIAnimationType.LEAVE));
                    }
                }
            }

            if (gameObject.activeInHierarchy)
                return l_allAnimCoroutine = StartCoroutine(WaitAnims(p_uiAnimationType, p_action));
